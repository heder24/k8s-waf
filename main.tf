...
resource "aws_cloudwatch_log_resource_policy" "knote-waf-policy" {
  policy_document = data.aws_iam_policy_document.knote-waf-policy.json
#   policy_document = file("./iam_policy.json")
  policy_name     = "webacl-policy-knote-waf"
}

data "aws_iam_policy_document" "knote-waf-policy" {
  version = "2012-10-17"
  statement {
    effect = "Allow"
    principals {
